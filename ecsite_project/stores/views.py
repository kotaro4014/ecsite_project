...
class ProductListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if product_type_name:     #商品タイプ絞り込み
            query = query.filter(
                product_type__name=product_type_name
            )
        if product_name:     #商品名絞り込み
            query = query.filter(
                name=product_name
            )
        order_by_price = self.request.GET.get('order_by_price', 0)
        if order_by_price == '1':
            query = query.order_by('price')     #昇順
        elif order_by_price == '2':
            query = query.order_by('-price')    #降順
        return query
...
class ConfirmOrderView(LoginRequiredMixin, TemplateView):
    ...
    @transaction.atomic     #原子性(一貫性) = 途中で例外が発生すればロールバックされる
